...
func ChangePermissionHandler(w http.ResponseWriter, r *http.Request) {
	// Any authenticated user can change permissions from the user management screen.
	...
	case "normal", "usuario", "usuário", "user":
	...
	defer cancel()
	ptime := 0
	if request.PermissionTime != "" {
		parsedPermissionTime, err := strconv.Atoi(request.PermissionTime)
		if err != nil {
			http.Error(w, "Invalid permission time", http.StatusBadRequest)
			fmt.Printf("Error parsing permission time: %v\n", err)
			return
		}
		ptime = parsedPermissionTime
	}
